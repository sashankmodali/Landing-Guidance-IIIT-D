function ydot = thirdtheta(t,y,zto,kmax)
%     global vtarray
    global stopdist flagarray timearray psidotarray vpmax desarray sarray tfinal vpdotarr vparr rarray rxyarray rdotxyarray controlvector controlmax angledesired crossnum So tuningparam varyingconstants Barray detarray Ainvstored tstored Bstored initialvals acc
    persistent tfinalflag visflag anotherflag alphato psio Rzo psiflag vpo vto psidoto alphatdoto vpdotprev changeflag Rxyo Rdotxyo avgalphatdot avgalphat
    ...
    alphat = y(1); vt = y(2); vp = y(3); gamma =y(4);alphap=y(5);xp=y(6);yp=y(7);zp=y(8);xt1=y(9);yt1=y(10);
...
%     arg =i;
    ...
    yt = yt1;
    xt = xt1;
%     angleused = wrapToPi((n-3*sign(angledesired))*pi/4);
%     if acc=="highmaneuvering"
%         yt = yt1 + 15*sin(angleused*sign(angledesired));
%         xt = xt1 + 15*cos(angleused*sign(angledesired));
%     else
%         yt=yt1;
%         xt=xt1;
%     end
%% 
    ...
    angled=0;
%     angled = wrapToPi((n+sign(angledesired))*pi/4);
...
%         angledesired = 0;
        ...
        if flag
            if Rxyo <= 40 
                ka = abs(5*(log(1+vpo))^0.2*(vpo-vto)/(vpo+vto)/Rxyo^(0.4*0.3));
            elseif Rxyo <=60
                ka = abs(5*(log(1+vpo))^0.2*(vpo-vto)/(vpo+vto)/Rxyo^(0.3*0.3));
            else
                ka = abs(5*(log(1+vpo))^0.2*(vpo-vto)/(vpo+vto)/Rxyo^0.08);
            end
        else
            if Rxyo <=10
            ...
            elseif Rxyo <=120
                ka = abs(5*((vpo-vto)/(vpo+vto))/Rxyo^0.4);
            end
        end
        ...
        if varyingconstants
            ...
            kc = (((nthroot(Rzo^2/Rxyo^2,3)))^flag)*1.5*ka;
        end
%         k=kmax;
        if isempty(So)
            So = [Rdotxy + ka*Rxy*nthroot(Rxy^2,-3)^flag;Rdotz+tan(thetadesired)*Rdotxy + kc*(Rz+tan(thetadesired)*Rxy)*nthroot(Rz^2,-3)^flag; psidot - alphatdot + kb*(wrapToPi(psi-alphat- angledesired))*nthroot(wrapToPi(psi-alphat-angledesired)^2,-3)^flag];
            if acc=="highmaneuvering"
                So = [Rdotxy + ka*Rxy*nthroot(Rxy^2,-3)^flag;Rdotz+tan(thetadesired)*Rdotxy + kc*(Rz+tan(thetadesired)*Rxy)*nthroot(Rz^2,-3)^flag; psidot + kb*(wrapToPi(psi- angled))*nthroot(wrapToPi(psi-angled)^2,-3)^flag];
            end
            So
...
%         B = [-k1*nthroot(S(1),3) - vtdot*cos(alphat-psi) + vp*sin(alphap-psi)*cos(gamma)+vt*sin(alphat-psi)*(alphatdot-psidot)+k*(vt*cos(alphat-psi)-vp*cos(alphap-psi)*cos(gamma)); - k2*nthroot(S(2),3)+tan(theta)*(vtdot*cos(alphat-psi)-vp*sin(alphap-psi)*cos(gamma)-vt*sin(alphat-psi)*(alphatdot-psidot));-k3*nthroot(S(3),3)+alphatdot];
        if flag
            ...
            S = [Rdotxy + ka*Rxy; Rdotz+tan(thetadesired)*Rdotxy + kc*(Rz+tan(thetadesired)*Rxy); psidot - alphatdot + kb*(wrapToPi(psi - alphat- angledesired))];
            if acc=="highmaneuvering"
                S = [Rdotxy + ka*Rxy;Rdotz+tan(thetadesired)*Rdotxy + kc*(Rz+tan(thetadesired)*Rxy); psidot + kb*(wrapToPi(psi - angled))];
            end
        end
        ...
        A = [1 0 0;tan(thetadesired) 1 0;0 0 1]*A;
...
%                 disp("WWWWWWWWWHHHHHHHHHHHHHAAAAAAAAAAAATTTTTTTTT!!")
            end
        end
        if varyingconstants
            kbterm = -2/3/nthroot(tan(theta),3)*sec(theta)^2*thetadot*ka*nthroot(Rz,3)*varyingconstants;
        else
            kbterm=0;
        end
        ...
        b2 =-k2*nthroot(S(2,1)^degreenum,degree) + kbterm + tan(thetadesired)*(vp*sin(alphap-psi)*cos(gamma)*psidot - vtdot*cos(alphat-psi)+vt*sin(alphat-psi)*(alphatdot-psidot)) - kc*(Rdotz + tan(thetadesired)*Rdotxy);
...
%             end
            ...
            gammadot = C(3);
            vpdot = C(1);
%             if vt*cos(alphat-psi)>0 && acc=="highmaneuvering"
%                 vpdot=0;
%                 gammadot=-wrapToPi(gamma-theta);
%                 alphapdot=-wrapToPi(alphap - psi);
%             end
...
%         amdot=0;
        ...
        disp(['Rxy Rz R', num2str([Rxy, Rz, R])]);
    end
    if vp>vpmax
        vpmax=vp;
    end
